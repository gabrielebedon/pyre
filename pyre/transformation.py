...
def pdf(x,marg):
  ...
  """
  p = getattr(sys.modules[__name__],getDistributionType(marg.getType())).pdf(x,marg.getP1(),marg.getP2(),marg.getP3(),marg.getP4())
  return p

def z_to_x(z,marg):
  """Transformation from z to x space"""
  x = getattr(sys.modules[__name__],getDistributionType(marg.getType())).u_to_x(z,marg)
  return x
...
def x_to_u(x,stochastic_model):
  ...
  for i in range(nrv):
    u[i] = getattr(sys.modules[__name__],getDistributionType(marg[i].getType())).x_to_u([x[i]],marg[i])
...
def u_to_x(u,stochastic_model):
  ...
  for i in range(nrv):
    x[i] = getattr(sys.modules[__name__],getDistributionType(marg[i].getType())).u_to_x([u[i]],marg[i])
  return x
...
def jacobian(u,x,stochastic_model):
  ...
  for i in range(nrv):
    J_u_x[i][i] = getattr(sys.modules[__name__],getDistributionType(marg[i].getType())).jacobian([u[i]],[x[i]],marg[i])
